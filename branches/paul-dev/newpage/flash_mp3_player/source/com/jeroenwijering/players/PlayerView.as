...
class com.jeroenwijering.players.PlayerView extends AbstractView { 
	...
	function PlayerView(ctr:AbstractController,car:Object,far:Array) { 
		super(ctr,car,far);
		configArray["fullscreenmode"] == "true" ? Stage.addListener(this): null;
	};
	...
	**/
	private function onLoad() {
		var ref = this;
		configArray["fullscreenmode"] == "true" ? configArray["displayheight"] = configArray["height"]: null;
		...
		if(configArray["displayheight"] == 0) { 
			...
			if(configArray["overstretch"] == "true") { 
				imageLoader = new ImageLoader(tgt.thumb,true,configArray["width"],configArray["displayheight"]);
			} else {
				imageLoader = new ImageLoader(tgt.thumb,false,configArray["width"],configArray["displayheight"]);
			}
		...
		tgt.scrub.bck.onPress = function() { 
			this.onEnterFrame = function() {
				var xm = this._parent._xmouse-this._parent.bar._x-2;
				if(xm < this._parent.bck._width-2) {
					this._parent.bar._width = xm;
					this._parent.icn._x = this._parent.bar._width + this._parent.bar._x + 1;
				}
			}
		};
		tgt.scrub.bck.onRelease = tgt.scrub.bck.onReleaseOutside = function() {
			var pct = (this._parent._xmouse-this._parent.bar._x)/ref.barWidths*100;
			ref.sendEvent("scrub",pct);
	...
	public function getUpdate(typ:String,pr1:Number,pr2:Number):Void {
		switch(typ) {
			...
			case "item":
				currentItem = pr1;
				configArray["displayheight"] > 0 ? loadThumb(currentItem): null;
				break;
			case "volume":
				setVolBtn(pr1);
				break;
	...
	/** 
	* State switch; show and hide the play/pause button and show activity icon
	*
	...
	private function switchState(stt:Number) {
		switch(stt) {
			case 0:
				...
				if (configArray["linkfromdisplay"] == "false" && configArray["displayheight"] > 0) {
	...
	*
	* @param elp	Amount of elapsed seconds
	* @param rem	Amount of remaining seconds
	**/
	private function setTimeBar(elp:Number,rem:Number) {
	...
	*
	* @param pct	percentage that is loaded
	**/
	private function setLoadBar(pct:Number) {
	...
	**/
	private function setVolBtn(pr1:Number) {
	...
	*
	* @param idx	fileArray index of the thumb to load
	**/
	private function loadThumb(idx:Number) {
	...
	/** 
	* Add a leading zero and convert number to string (used by setTimeBar)
	*
	* @param nbr	Number to check and convert.
	**/
